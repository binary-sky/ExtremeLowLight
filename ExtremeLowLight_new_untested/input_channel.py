...
def _main_pre_process(X, Y, ratio, classID, time, iso):
    ...
    X_img = X_img.astype(np.float32)                        # float32
    Y_img = Y_img.astype(np.float32)                        # float32
    
    if np.equal(classID, 0):    # if classID == 0:          # 
        X_img = np.maximum(X_img - 512, 0) / (16383 - 512)
    elif np.equal(classID, 1):  # if classID == 1:          # 
        X_img = X_img / 959.0

       
    Y_img = Y_img / 255.0 #8 bit
...
def _readX(X_path):
    ...
    X_img = X_img[:, :, raw_pattern_reverse]  # channel : R G1 B G2
...
def read_image_from_file(X, Y): 
    # out<1> :  4 channel half image tensor，uint16
    # out<2> :  3 channel full image tensor，uint16
    if pa.buff == True:
        ...
        if os.path.exists(X_buff_loc):
            try:
                ...
                print('cannot read file：',X_buff_loc)
        else:
        ...
        if not pa.testing_now:
            if os.path.exists(YfX_buff_loc):
                try:
                    ...
                    print('cannot read file：',Y_buff_loc)
            else:
                Y_img = _readYfX(Y,X)
