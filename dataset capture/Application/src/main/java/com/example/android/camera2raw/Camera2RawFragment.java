...
public class Camera2RawFragment extends Fragment
    ...
    private final ImageReader.OnImageAvailableListener mOnJpegImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            Log.e("505030475","完成 -- mOnJpegImageAvailableListener  onImageAvailable"+(cnt+1));
            xreader = reader;
            //if(cnt%2==1){
                Log.e("505030475","存储一幅RAW图像");
                ...
                ImageSaverX saver = new ImageSaverX(xImage, mFile, tor, mCharacteristics, activity);
                AsyncTask.THREAD_POOL_EXECUTOR.execute(saver);
           // }else{
           ///     Log.e("505030475","舍弃一幅图像");
           //     Image xImage = xreader.acquireNextImage();
           // }
            cnt++;
    ...
    private final ImageReader.OnImageAvailableListener mOnRawImageAvailableListener
        ...
        @Override
        public void onImageAvailable(ImageReader reader) {
            Log.e("505030475","完成 -- mOnRawImageAvailableListener  mRawImageReader");
            Log.e("505030475","存储一幅RAW图像");
            ...
            ImageSaverX ysaver = new ImageSaverX(yImage, yFile, tor, mCharacteristics, activity);
            AsyncTask.THREAD_POOL_EXECUTOR.execute(ysaver);
    ...
    private CameraCaptureSession.CaptureCallback mPreCaptureCallback
            ...
            synchronized (mCameraStateLock) {
                switch (mState) {
                    ...
                    case STATE_WAITING_FOR_3A_CONVERGENCE: {
                        Log.e("505030475","case STATE_WAITING_FOR_3A_CONVERGENCE");
                        //检查3A状态
                        ...
                        Log.e("505030475","readyToCapture:"+readyToCapture);
                        //只有当按键有计数时才进行下一步
                        ...
                        if(readyToCapture && burst_confirm ){
                            Log.e("505030475","captureStillPictureLockedXX(mISO,mSS)");

                            captureStillPictureLockedXX(mISO,mSS);
                            burst_confirm=false;
                            // After this, the camera will go back to the normal state of preview.
        ...
        public void onCaptureProgressed(CameraCaptureSession session, CaptureRequest request,
            ...
            Log.e("505030475","onCaptureProgressed");
    ...
    private final CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
                                       TotalCaptureResult result) {
            Log.e("505030475","完成 -- onCaptureCompleted");
    ...
    SeekBar seekbar_time_high;
    TextView tv_time_high,tv_time_low;

    SeekBar seekbar_time_low;
    ...
    SeekBar.OnSeekBarChangeListener iso_listenner = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress,
                                      boolean fromUser) {
            tv_iso.setText("ISO->" + sb_iso.getProgress());
            synchronized (mCameraStateLock) {
                ...
                try {
                    ...
                    setup2AControlsLocked(mPreviewRequestBuilder,sb_iso.getProgress(),get_ss(seekbar_time_low));
    ...
    SeekBar.OnSeekBarChangeListener ss_listenner = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress,
                                      boolean fromUser) {
            if(seekBar==seekbar_time_low)
                tv_time_low.setText("LOW->" + (float)get_ss(seekBar)/1000000000);
            else
                tv_time_high.setText("HIGH->" + (float)get_ss(seekBar)/1000000000);
            ...
            synchronized (mCameraStateLock) {
                ...
                try {
                    setup2AControlsLocked(mPreviewRequestBuilder,sb_iso.getProgress(),get_ss(seekBar));
                    // Finally, we start displaying the camera preview.
                    mCaptureSession.setRepeatingRequest(
                            mPreviewRequestBuilder.build(),
                            mPreCaptureCallback, mBackgroundHandler);
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.burst1).setOnClickListener(this);
        view.findViewById(R.id.burst2).setOnClickListener(this);
        tv_debug = view.findViewById(R.id.textView3);

        sb_iso = view.findViewById(R.id.seekBar);
        tv_iso = view.findViewById(R.id.textView);
        seekbar_time_high = view.findViewById(R.id.seekBar3);
        tv_time_high = view.findViewById(R.id.textView4);
        tv_time_low = view.findViewById(R.id.textView2);

        seekbar_time_low = view.findViewById(R.id.seekBar2);

        SharedPreferences spp = getActivity().getSharedPreferences("position",Context.MODE_PRIVATE);
        seekbar_time_low.setProgress(spp.getInt("seekbar_time_low",20));
        seekbar_time_high.setProgress(spp.getInt("seekbar_time_high",45));
        tv_iso.setText("ISO->"+sb_iso.getProgress());


        tv_time_low.setText("LOW->" + (float)get_ss(seekbar_time_low)/1000000000);

        tv_time_high.setText("HIGH->" + (float)get_ss(seekbar_time_high)/1000000000);


        sb_iso.setOnSeekBarChangeListener(iso_listenner);
        seekbar_time_high.setOnSeekBarChangeListener(ss_listenner);
        seekbar_time_low.setOnSeekBarChangeListener(ss_listenner);
        ...
        music = sp.load(getActivity().getApplication(), R.raw.rockdrums, 1);
    ...
    private void sleeeper(long time ) {
        try {
            Thread.sleep(time);
    ...
    int locate(double target){
        for(int i=0;i<expsure_time_list.length;i++){
            Log.e("505030475", "i="+i+"   expsure_time_list[i]="+expsure_time_list[i]+
                    "   Math.abs(expsure_time_list[i]-target)="+
                    Math.abs(expsure_time_list[i]-target));

            if(Math.abs(expsure_time_list[i]-target)<0.00001)
            {
                return i;
            }
        }
    ...
    void save_loacation(){
        ...
        spe.putInt("seekbar_time_low",seekbar_time_low.getProgress());
        spe.putInt("seekbar_time_high",seekbar_time_high.getProgress());
        spe.apply();
    ...
    double [] time_set_exp(long low,long high){
        ...
        Log.e("505030475", "low_time:"+low_time+"high_time"+high_time);

        int low_time_loacation = locate(low_time);
        int hight_time_loacation = locate(high_time);
        Log.e("505030475", "low_time_loacation:"+low_time_loacation+"hight_time_loacation"+hight_time_loacation);

        if(hight_time_loacation-low_time_loacation+1<8){
            EXPOSURE_TIMES=9; //avalible = 8
            double [] exp_set = new double[EXPOSURE_TIMES];
            for(int i=0;i<8;i++){
                exp_set[7-i] = expsure_time_list[low_time_loacation+i];
            }
            exp_set[8]=exp_set[7];
            return exp_set;
        }
            ...
            for(int i=0;i<8;i++){
                ...
                exp_set[7-i] = expsure_time_list[index];
            }
            exp_set[8]=exp_set[7];
            return exp_set;
        }
    ...
    void ready_take_burst(){
        ...
        mKeyID = generateTimestamp();//产生时戳

        final int ISO_para = sb_iso.getProgress();          //获取曝光参数
        final double [] expsure_time_set =                //获取曝光参数
                time_set_exp(get_ss(seekbar_time_low),get_ss(seekbar_time_high));//获取曝光参数



        Thread th = new Thread(new Runnable() {//新建线程进行连拍任务
            @Override
            public void run() {
                defaultMediaPlayer();//播放声音 延迟几秒
                takeBurst(expsure_time_set,ISO_para);
            }
        });
        th.run();
    ...
    private void takeBurst(double [] expsure_time_set,int ISO_para) {
        save_loacation();
        burst_requests= new ArrayList<>();
        cnt=0;
        mmcnt=0;
        ImageSaverX.count_down = (EXPOSURE_TIMES*2)*2;
        ...
        try {
            for (int i=0;i<(EXPOSURE_TIMES*2);i++){
                ...
                captureBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME,(long)(expsure_time_set[j]*1000000000));//单位纳秒
                Log.e("505030475", "设置ISO为"+ISO_para+"设置曝光时间为"+expsure_time_set[j]);
    ...
    public class TimerTaskTest01 extends TimerTask{
        public void run() {
            Log.e("505030475","当前"+ImageSaverX.count_down);
            tv_debug.post(new Runnable() {
                @Override
                public void run() {
                    tv_debug.setText("当前"+ImageSaverX.count_down);
                }
            });
            if(ImageSaverX.count_down==0){
                timer.cancel();
                terminal_process();
                boolean ohyeah=delate_extral();
                if(ohyeah){
    ...
    private boolean delate_extral()
    {
        ...
        for(int j=0;j<filelist.size();j++)
        {
            if(save_index.indexOf(j)==-1){
                Log.e("505030475","删除"+filelist.get(j));
                Log.e("505030475","删除"+filelist_raw.get(j));
    ...
    private final CameraCaptureSession.CaptureCallback xCaptureCallbcak
        ...
        public void onCaptureStarted(CameraCaptureSession session, CaptureRequest request,
                                     long timestamp, long frameNumber) {
            Log.e("505030475","完成 -- onCaptureStarted");

        }

        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
                                       TotalCaptureResult result) {
            Log.e("505030475","曝光时间"+String.valueOf((float)(request.get(CaptureRequest.SENSOR_EXPOSURE_TIME))/1000000000)
                    +"实际曝光时间"+
    ...
    public static ArrayList<File> getFileList_raw(String strPath) {
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                ...
                } else if (fileName.endsWith("dng")) { // 判断文件名是否以.avi结尾
    ...
    private void takePicture_Single_simple(float time,int ISO,int tagger)
    {
        ...
        try {
            ...
            captureBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME,(long)(time*1000000000));//单位纳秒
            // If there is an auto-magical white balance control mode available, use it.
            if (contains(mCharacteristics.get(
    ...
    private void createCameraPreviewSessionLocked() {
        try {
            ...
            mPreviewRequestBuilder.addTarget(surface);//输出指定
            // mPreviewRequestBuilder.removeTarget(surface);//输出指定
    ...
    private void takePicture_Single(float time,int ISO) {
        ...
        //线程锁
        synchronized (mCameraStateLock) {
            Log.e("505030475", "takePicture_Single");
            //为了方便设定一个全局值
            mISO = ISO;
            mSS = time;
            ...
            //////??????????////////?????????????????????
//            setup2AControlsLocked(mPreviewRequestBuilder,mISO,(long)(mSS*1000000000));
//            // Finally, we start displaying the camera preview.
//            try {
//                mCaptureSession.setRepeatingRequest(
//                        mPreviewRequestBuilder.build(),
...
//            }
            ...
            try {
                if (!mNoAFRun) {
                    ...
                    // 等待计时 Start a timer for the pre-capture sequence.
                    startTimerLocked();

                    // Replace the existing repeating request with one with updated 2A triggers.
                    // 置换采集请求
                    Log.e("505030475", "mCaptureSession.capture(mPreviewRequestBuilder.build(), mPreCaptureCallback, mBackgroundHandler)");
                    mCaptureSession.capture(mPreviewRequestBuilder.build(), mPreCaptureCallback, mBackgroundHandler);
                    Log.e("505030475", "mCaptureSession.capture OK");
                }
            }catch (CameraAccessException e) {
                e.printStackTrace();
            }
    ...
    private void captureStillPictureLockedXX(int ISO,float time) {
        try {
            final Activity activity = getActivity();
            if (null == activity || null == mCameraDevice) {
                return;
    ...
    private static class ImageSaver implements Runnable {
        ...
        public static class ImageSaverBuilder {
            ...
            private boolean isComplete() {
                ...
                Log.e("505030475","-------------isComplete-------------");
                Log.e("505030475","mImage:"+(mImage != null));
                Log.e("505030475","mFile:"+(mFile != null));
                Log.e("505030475","mCaptureResult:"+(mCaptureResult != null));
                Log.e("505030475","mCharacteristics:"+(mCharacteristics != null));
                Log.e("505030475","-------------isComplete-------------");
    ...
    private static class ImageSaverX implements Runnable {
        ...
        private ImageSaverX(Image image, File file, CaptureResult result,
                           CameraCharacteristics characteristics, Context context) {
            mImage = image;
            mFile = file;
            mCaptureResult = result;
    ...
    private void handleCompletionLocked(int requestId, ImageSaver.ImageSaverBuilder builder,
        ...
        Log.e("505030475","handleCompletionLocked");

        Log.e("505030475","requestId："+requestId);
        if (builder == null) return;
        ImageSaver saver = builder.buildIfComplete();
        Log.e("505030475","saver == null  ？？？");
        if (saver != null) {
            Log.e("505030475","saver ！= null  ！！");
    ...
    long get_ss(SeekBar seekbar_time_high)
    {
        int barValue =  seekbar_time_high.getProgress();
